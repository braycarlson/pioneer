...
class OpenCVErosion(OpenCVDialog):
    def __init__(self, identifier: str):
        ...
        self.setWindowTitle('OpenCV: Erosion')
    ...
    def create(self) -> None:
        callback = [self.push]
        self.erosion_widget = KernelWidget(callback)
    ...
    def send(self) -> dict[str, Any]:
        return {
            ...
            'name': self.name,
            'parameter': {
                'kernel_size': self.erosion_widget.slider.value(),
                'iterations': self.iteration_widget.slider.value()
            },
            'signal': 'opencv_erosion'
        }
